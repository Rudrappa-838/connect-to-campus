...
exports.addStaff = async (req, res) => {
    ...
    try {
        ...
        let schoolPrefix;
        if (customPrefix) {
            schoolPrefix = customPrefix.toUpperCase();
        } else {
            schoolPrefix = schoolName.replace(/[^a-zA-Z]/g, '').substring(0, 2).toUpperCase();
        ...
        if (providedId) {
            ...
            while (!isUnique) {
                ...
                const checkStaff = await client.query('SELECT id FROM staff WHERE employee_id = $1', [employee_id]);
    ...
    try {
        ...
        const result = await client.query(
            `UPDATE public.staff SET name = $1, email = $2, phone = $3, role = $4, gender = $5, address = $6, join_date = $7, salary_per_day = $8,
             ...
             manual_attendance_classes = COALESCE($18::jsonb, manual_attendance_classes)
