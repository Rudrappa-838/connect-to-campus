...
if (!dbConfig.user || !dbConfig.host || !dbConfig.database || !dbConfig.password) {
    ...
    // Attempt fallback to DATABASE_URL if discrete vars failed but URL exists
...
async function fixData() {
    ...
    const pool = new Pool(process.env.DATABASE_URL ? { connectionString: process.env.DATABASE_URL, ssl: { rejectUnauthorized: false } } : dbConfig);
