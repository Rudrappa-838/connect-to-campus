...
const TeacherGeofenceLogs = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            const [logsRes, dailyRes] = await Promise.all([
                api.get('/teachers/attendance/geofence-logs', {
                    params: { date, teacherId: selectedTeacherId || undefined }
                }),
                api.get('/teachers/attendance/daily', {
                    params: { date }
                })
            ]);
    ...
    const teacherSummaries = dailyAttendance.map(teacher => {
        const teacherLogs = logs.filter(l => l.teacher_id === teacher.id);
        
        // Calculate session count (number of check-ins)
        const checkInCount = teacherLogs.filter(l => l.event_type === 'CHECK_IN').length;
        
        return {
            ...teacher,
            logs: teacherLogs,
            checkInCount,
            totalLogs: teacherLogs.length
        };
    }).filter(summary => {
        // If filter is active, only return the selected teacher
        if (selectedTeacherId) {
            return summary.id === parseInt(selectedTeacherId);
        }
        return true;
    });
    ...
    return (
        <div className="space-y-6 animate-in fade-in">
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 bg-white p-5 rounded-2xl shadow-sm border border-slate-200">
                <div>
                    <h2 className="text-xl font-bold text-slate-800">Geofence Check-In/Out Audit Logs</h2>
            ...
            {loading ? (
                ...
                <div className="space-y-4">
                    {teacherSummaries.map(summary => {
                        const isExpanded = expandedTeacherId === summary.id;
                        ...
                        return (
                            ...
                            >
                                ...
                                <div 
                                    ...
                                    onClick={() => setExpandedTeacherId(isExpanded ? null : summary.id)}
                                >
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-slate-400">Contact: {summary.phone || 'N/A'}</p>
                                        </div>
                                    </div>
...
export default TeacherGeofenceLogs;
